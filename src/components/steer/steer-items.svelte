...
<ul class="relative text-center font-display text-lg {className}">
  {#each steerItems as item}
    <li class="relative">
      <a class="text-2xl no-underline lg:text-sm xl:text-base" href={item.href}>
        <div
          class="bg-red-20 relative mx-auto mb-1.5 h-fit w-fit p-1 
          px-3 pt-1.5 pb-0.5 lg:bg-transparent lg:px-0"
        >
          {item.text}
        </div>
      </a>
...
<style lang="postcss">
  /* li::before,
  li:last-child:after {
    @apply mx-1 my-[1em] inline-block h-[.625em] w-[0.625em] bg-yellow align-middle content-[""];
